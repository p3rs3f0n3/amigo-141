...
python -m venv ami_venv
ami_venv\Scripts\activate        # Windows
# source ami_venv/bin/activate   # Mac/Linux
...
Los datasets se cargan en memoria al iniciar el servidor y no se actualizan en vivo. Si los datos de `datos.gov.co` cambian, basta con reiniciar el servidor.

Para producción se recomienda programar un reinicio automático nocturno (cuando el tráfico es bajo) usando un cron job o el task scheduler de Windows:

```bash
# Linux/Mac — cron job a las 2am
0 2 * * * systemctl restart amigo-backend

# Windows — Task Scheduler apuntando a:
